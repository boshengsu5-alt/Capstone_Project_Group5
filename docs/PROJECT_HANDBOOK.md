...
| 设备丢失 | — | **1.0** | 归还时设备缺失 |
| 故意破坏 | — | **1.0 + 纪律处分** | 主观故意损毁，不走折旧逻辑，按净值全额赔偿 |

#### 计算示例

> **场景**：MacBook Pro（原值 ¥15,000，购置 2 年），归还时发现屏幕破裂（中度损坏）。
>
...
> 系统中已有 `profiles.credit_score` 字段（满分 100 分），以下为损坏/违规行为对应扣分标准。
...
损坏报告由管理员在 Web 端审核确认后，系统按以下流程自动推进：
...
| 损坏等级 | `severity` (`minor`/`moderate`/`severe`) | `damage_reports` |
